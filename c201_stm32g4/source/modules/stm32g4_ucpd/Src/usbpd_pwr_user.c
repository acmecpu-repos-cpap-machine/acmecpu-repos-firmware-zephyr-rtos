...
#else
extern struct vbus_ucpd_sense g_vbus_sense;
#endif	/* DUMMY_FOR_BUILD */
...
static uint32_t raw_to_mvolts(uint16_t raw, uint8_t resolution) {
	int multip = 256;
	...
	uint32_t mvolts = (raw * (3300) / multip);
	return mvolts;
}
...
#define BAD_ANALOG_READ -123
	uint16_t adc_buff=0;
	g_vbus_sense.seq.buffer = &adc_buff;
	g_vbus_sense.seq.buffer_size = sizeof(adc_buff);

	ret = adc_channel_setup(g_vbus_sense.dev, &g_vbus_sense.chcfg);
	if (ret != 0) {
		return ret;
	}

	ret = adc_read(g_vbus_sense.dev, &g_vbus_sense.seq);
	if (ret != 0) {
		return ret;
	}

	uint32_t voltage = 0;
	voltage = raw_to_mvolts(adc_buff, g_vbus_sense.seq.resolution);
	voltage = ((voltage * (390 + 75)) / 75);
//    voltage /= 1000U;
