...
/** @defgroup USBPD_USER_PRIVATE_DEFINES USBPD USER Private Defines
  * @{
  */
#define DPM_APP_NOTIF_ISCONNECTED       (1 << 5)
#define DPM_APP_NOTIF_POWER_EVENT       (1 << 15)
...
stm32g4_ucpd_post_notif_msg_handler_t m_post_notif_handler;
stm32g4_ucpd_fmt_send_notif_handler_t m_fmt_send_notif_handler;
stm32g4_ucpd_save_info_handler_t m_save_info_handler;
...
/**
  * @brief  Function to set the function ptr linked to application interface
  ...
  * @retval None
  */
void USBPD_DPM_SetNotification_APP(
	...
	m_post_notif_handler = PtrPost;
	m_fmt_send_notif_handler = PtrFormatSend;
	m_save_info_handler = PtrSaveInfo;
}
...
void USBPD_DPM_UserCableDetection(uint8_t PortNum, USBPD_CAD_EVENT State)
{
	...
	case USBPD_CAD_EVENT_ATTACHED:
		...
		if (NULL != m_fmt_send_notif_handler) {
			m_fmt_send_notif_handler(PortNum,
					DPM_APP_NOTIF_ISCONNECTED, 0);
		}
		break;
	default:
		/* Format and send a notification to GUI if enabled */
		if (NULL != m_fmt_send_notif_handler) {
			m_fmt_send_notif_handler(PortNum,
					DPM_APP_NOTIF_ISCONNECTED | DPM_APP_NOTIF_POWER_EVENT, 0);
		}
	}
#endif
}
...
/* USER CODE BEGIN USBPD_DPM_Notification */
	...
	if (NULL != m_post_notif_handler) {
		m_post_notif_handler(PortNum, EventVal);
	}
...
/* USER CODE END USBPD_DPM_SetDataInfo */
  ...
  if (NULL != m_save_info_handler)
  {
	  m_save_info_handler(PortNum, DataId, Ptr, Size);
  }
}
