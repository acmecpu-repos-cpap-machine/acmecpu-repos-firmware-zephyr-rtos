...
/*Passing device structure*/
const struct device *const dev = DEVICE_DT_GET_ONE(bosch_bmi270);
static struct i2c_dt_spec intf_conf;

BMI2_INTF_RETURN_TYPE bmi2_i2c_read(uint8_t addr, uint8_t *reg_data, uint32_t len, void * intf_ptr)
{
	struct bmi270_config *bmi_config = (struct bmi270_config *) dev->config;
	//struct i2c_dt_spec *i2c = &bmi_config->bus.i2c;
	return i2c_burst_read_dt(&bmi_config->bus.i2c , addr, reg_data, len);
}

BMI2_INTF_RETURN_TYPE bmi2_i2c_write(uint8_t addr, const uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
	struct bmi270_config *bmi_config = (struct bmi270_config *) dev->config;
	//struct i2c_dt_spec *i2c = &bmi_config->bus.i2c;
	return i2c_burst_write_dt(&bmi_config->bus.i2c, addr, reg_data, len);
}
...
/*!
 *  @brief Function to select the interface bcoines_intf_configetween SPI and I2C.
 */
