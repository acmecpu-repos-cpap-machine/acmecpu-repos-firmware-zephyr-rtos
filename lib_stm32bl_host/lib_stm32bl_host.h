...
typedef int (*usart_send_t)(const void *data, int len);
typedef int (*usart_recv_t)(void* buf, int length, int ms_to_wait);
typedef int (*usart_close_t)();
typedef void (*ms_delay_t)(uint32_t ms);

struct lib_stm32bl_host_funcs {
	/**
	 * Function signature:
 ...
 *
 * @param:	stm32_ubl	pointer to struct lib_stm32bl_host_funcs having function pointers to the systems's usart driver functions
