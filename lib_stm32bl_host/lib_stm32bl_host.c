...
#include <string.h>
#include <stdlib.h>
#include "lib_stm32bl_host_config.h"
#include "lib_stm32bl_host.h"

#if PLATFORM_ESP_IDF
	#include "driver/uart.h"
	#include "esp_log.h"
	#define TAG	"lib_stm32bl_host"
#endif

#if PLATFORM_ZEPHYR
	#include <zephyr/kernel.h>
	#include <zephyr/logging/log.h>
	LOG_MODULE_REGISTER(lib_stm32bl_host);
#endif
...
static struct lib_stm32bl_host_funcs m_ubl_comm = { NULL, NULL, NULL, NULL, NULL };
static int m_stm32bl_host_ready = NOT_READY;
...
/********************************************************************************************/
int lib_stm32bl_host_init(struct lib_stm32bl_host_funcs *stm32_ubl)
{
	int ret = 0;
	m_stm32bl_host_ready = NOT_READY;
...
int lib_stm32bl_host_usart_open()
{
	int ret = -1;
...
/************************************* START BOOTLOADER *************************************/
/********************************************************************************************/
int lib_stm32bl_host_start_check()
{
	if ((m_ubl_comm.usart_send == NULL) || (m_ubl_comm.usart_recv == NULL) || (m_ubl_comm.ms_delay == NULL)) {
		return -1;
	}
...
/**************************************** GET COMMAND ***************************************/
/********************************************************************************************/
int lib_stm32bl_host_cmd_execute_get(uint8_t *p_resp, size_t *p_len)
{
...
/************************************ GET VERSION COMMAND ***********************************/
/********************************************************************************************/
int lib_stm32bl_host_cmd_execute_get_version(uint8_t *bl_ver, uint8_t *ob_1, uint8_t *ob_2)
{
...
/**************************************** GO COMMAND ****************************************/
/********************************************************************************************/
int lib_stm32bl_host_cmd_execute_go()
{
	if (check_lib_status() != 0)
		return -1;

	uint8_t txdata[2] = { 0x00 };
	uint8_t start_addr[4] = STM32BLH_CONFIG_STM32_APP_START_ADDR;
...
/********************************** READ MEMORY COMMAND *************************************/
/********************************************************************************************/
int lib_stm32bl_host_cmd_execute_read_mem(uint8_t *start_addr, uint8_t *buf, size_t bytes_to_read)
{
	if ( check_lib_status() != 0 )	return -1;

	int ret = 0, rx_num = 0, tx_num = 0;
#if 0//DEBUG_LOG
	printf("start_addr: [0]:[0x%x] [1]:[0x%x] [2]:[0x%x] [3]:[0x%x]\n",
	...
	if (tx_num == len) {
		...
		/* BYTE 3 to 6: start address */
#if DEBUG_LOG
		printf("transmitting start_addr: [0]:[0x%x] [1]:[0x%x] [2]:[0x%x] [3]:[0x%x]\n",
					start_addr[0], start_addr[1], start_addr[2], start_addr[3]);
#endif
...
#if DEBUG_LOG
		printf("transmitting checksum [0x%x]\n", checksum);
#endif
		tx_num = m_ubl_comm.usart_send((const char*) &checksum, sizeof(checksum));
		if (tx_num != sizeof(checksum)) {
			printf("usart_send start_addr checksum, tx_num = %d\n", tx_num);
		...
		uint8_t nob[2];
		nob[0] = bytes_to_read - 1U;	// max 256 bytes
		nob[1] = nob[0] ^ 0xFFU;

#if DEBUG_LOG
		printf("transmitting nob [0x%x] nob checksum [0x%x]\n", nob[0], nob[1]);
#endif
...
/********************************************************************************************/
int lib_stm32bl_host_cmd_execute_write_mem(uint8_t *start_addr, uint8_t *buf, size_t bytes_to_write)
{
	if ( check_lib_status() != 0 )	return -1;
...
/********************************************************************************************/
int lib_stm32bl_host_cmd_execute_erase_mem(uint8_t num_pages, uint8_t start_page, uint8_t end_page, uint8_t mass_erase)
{
	if ( check_lib_status() != 0 )	return -1;
...
/********************************************************************************************/
int lib_stm32bl_host_cmd_execute_ext_erase_mem(uint16_t nop, uint8_t *pgnos, uint8_t *spl_erase)
{
	if ( check_lib_status() != 0 )	return -1;
