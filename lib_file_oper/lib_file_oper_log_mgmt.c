...
static struct k_sem m_log_mgmt_lock;
...
static int get_oldest_filename_with_path(const char *searchpath, const char *curr_fname, char *out_name_path) {
	...
	int ret = 0;
	struct fname_node *fname_node = NULL, *tmp = NULL;
	...
	ret = lib_file_oper_helper_file_list_get(searchpath, &fname_list);
...
//		}
//	}
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&fname_list, fname_node, tmp, node)
//	SYS_SLIST_FOR_EACH_NODE_SAFE(&fname_list, fname_node, tmp)
	{
		if ((fname_node) && (!strcmp(curr_fname, fname_node->fname))) {
//			sys_slist_remove(&fname_list, NULL, &fname_node->node);
			sys_slist_find_and_remove(&fname_list, &fname_node->node);
			free(fname_node);
		}
	}
...
int lib_file_oper_helper_perform_data_rotation(struct data_rotation *dr, struct fs_file_t *zfp) {
	...
	ret = lib_file_oper_helper_get_file_size(curr_fname_path, &file_size_bytes);
	if ((ret == 0) && (file_size_bytes >= curr_max_fsize)) {
		/* Max size reached, check log file count */
		uint32_t log_file_count = 1;
		ret = lib_file_oper_helper_count_files(oper_dir, 0, &log_file_count);
		if (log_file_count >= max_fcount) {
			...
			ret = lib_file_oper_helper_delete_file(oldest_file);
		}
...
//		fs_close(zfp);
		...
		ret = lib_file_oper_helper_file_rename(curr_fname_path, new_filename);
		ret = 1;	// log file rotated, caller must open a new file
	}
