...
int codec_shutdown()
{
    api->param_set(codec_dev, CMX655_PREAMP, 0x00);
    api->param_set(codec_dev, CMX655_VOLUME, 0x80);

    k_sleep(K_MSEC(100));    // wait for volume settings

    api->param_set(codec_dev, CMX655_SYSCTRL, 0x00);
    codec_stopclk();
    return 0;
}

int codec_enable_streams(uint8_t num_chan, uint8_t rec_pb)
{
    int ret = 0;
    uint8_t saictrl=0, saimux=0, sysctrl=0x20;
    uint8_t level=0, preamp=0, volume=0;
#if (CMX655D_EXT_OCS_CLK)
    if (num_chan == 1)          saictrl = 0xB8;
    else if (num_chan == 2)     saictrl = 0x98;
    else                        return -EINVAL;
    
    if (rec_pb & (LIB_AUDIO_RECORD)) {
        saimux = 0x00;
        if (num_chan == 1) {
            // level = 0xC0;   // 0dB gain left channel
            level = 0xF0;   // 3dB gain left channel
            sysctrl |= 0x02;
        }
        if (num_chan == 2) {
            // level = 0xCC;   // 0dB gain left and right channel
            level = 0xFF;   // 3dB gain left and right channel
            sysctrl |= 0x03;
        }
    } 
    
    if (rec_pb & (LIB_AUDIO_PLAYBACK)) {
        if (num_chan == 1)  saimux &= 0xF7;
        if (num_chan == 2)  saimux |= 0x08;
        // preamp = 0x01;      // 6dB preamp gain
        // preamp = 0x02;      // 12dB preamp gain
        preamp = 0x03;      // 18dB preamp gain
        volume = 0x7F;      // 0dB (max volume)
        sysctrl |= 0x08;
    }

    if (rec_pb & (LIB_AUDIO_PLAYBACK_LOUT)) {
        sysctrl |= 0x10;
    }

    /* set the value */
    api->param_set(codec_dev, CMX655_SAICTRL, saictrl);
    api->param_set(codec_dev, CMX655_SAIMUX, saimux);
    if (rec_pb & (LIB_AUDIO_RECORD))
        api->param_set(codec_dev, CMX655_LEVEL, level);
    if (rec_pb & (LIB_AUDIO_PLAYBACK | LIB_AUDIO_PLAYBACK_LOUT)) {
        api->param_set(codec_dev, CMX655_PREAMP, preamp);
        api->param_set(codec_dev, CMX655_VOLUME, volume);
    }
    api->param_set(codec_dev, CMX655_SYSCTRL, sysctrl);
