...
#define DT_DRV_COMPAT nxp_pcal9539a

#define ACPU_ENABLE		0
/**
 * @file Driver for PCAL9539A I2C-based GPIO driver.
 */
...
LOG_MODULE_REGISTER(gpio_pcal9539a);
...
struct gpio_pcal9539a_drv_data {
	...
	struct {
		...
		/* variables to enable/disable interrupts for each input pin */
 ...
 *
 * @param dev Device struct of the PCAL9539A.
...
{
	const struct gpio_pcal9539a_config * const config = dev->config;
	// struct gpio_pcal9539a_drv_data * const drv_data =
	// 	(struct gpio_pcal9539a_drv_data * const)dev->data;
	...
	ret = i2c_burst_read_dt(&config->i2c_master, reg, 
				 (uint8_t *)&port_data, sizeof(port_data));
	if (ret != 0) {
		LOG_ERR("PCAL9539A[0x%X]: error reading register 0x%X (%d)",
			config->i2c_master.addr, reg, ret);
		return ret;
	}
	...
	*buf = value;
	...
	LOG_DBG("PCAL9539A[0x%X]: Read: REG[0x%X] = 0x%X, REG[0x%X] = 0x%X",
		config->i2c_master.addr, reg, (*buf & 0xFF), (reg + 1), (*buf >> 8));
			   ...
			   uint16_t *cache, uint16_t value)
{
	...
	LOG_DBG("PCAL9539A[0x%X]: Write: REG[0x%X] = 0x%X, REG[0x%X] = "
		"0x%X", config->i2c_master.addr, reg, (value & 0xFF),
		(reg + 1), (value >> 8));
	...
	// 		      (uint8_t *)&port_data, sizeof(port_data));
	ret = i2c_burst_write_dt(&config->i2c_master, reg, 
				  (uint8_t *)&port_data, sizeof(port_data));
	if (ret == 0) {
		*cache = value;
	} else {
		LOG_ERR("PCAL9539A[0x%X]: error writing to register 0x%X "
			"(%d)", config->i2c_master.addr, reg, ret);
	}
 ...
 * @return 0 if successful, failed otherwise
 */
static int gpio_pcal9539a_pin_configure(const struct device *dev,
			       gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if (ret) {
		LOG_ERR("PCAL9539A[0x%X]: error setting pin direction (%d)",
			i2c_addr, ret);
		goto done;
	}
	...
	if (ret) {
		LOG_ERR("PCAL9539A[0x%X]: error setting pin pull up/down "
			"(%d)", i2c_addr, ret);
		goto done;
	}
...
static int gpio_pcal9539a_port_get_raw(const struct device *dev,
				     uint32_t *value)
{
	struct gpio_pcal9539a_drv_data * const drv_data =
		(struct gpio_pcal9539a_drv_data * const)dev->data;
...
static int gpio_pcal9539a_port_set_masked_raw(const struct device *dev,
...
static int gpio_pcal9539a_port_set_bits_raw(const struct device *dev,
					  uint32_t mask)
{
	return gpio_pcal9539a_port_set_masked_raw(dev, mask, mask);
}

static int gpio_pcal9539a_port_clear_bits_raw(const struct device *dev,
					    uint32_t mask)
{
	return gpio_pcal9539a_port_set_masked_raw(dev, mask, 0);
}

static int gpio_pcal9539a_port_toggle_bits(const struct device *dev,
					 uint32_t mask)
{
	struct gpio_pcal9539a_drv_data * const drv_data =
		(struct gpio_pcal9539a_drv_data * const)dev->data;
...
{
	struct gpio_pcal9539a_drv_data * const drv_data = CONTAINER_OF(
		work, struct gpio_pcal9539a_drv_data, interrupt_worker);

	uint16_t input_new, input_cache, changed_pins=0;
	int ret;

#if 1//ACPU_ENABLE
	uint16_t trig_level = 0, trig_edge = 0;
	...
	/* Update and store the pin values again, so that we can detect the changed pins next time */
//	update_input_regs(drv_data->instance, &input_new);

	k_sem_give(&drv_data->lock);

	/* Fire callbacks */
//	gpio_fire_callbacks(&drv_data->callbacks, drv_data->instance, changed_pins);
...
{
	struct gpio_pcal9539a_drv_data * const drv_data =
		CONTAINER_OF(cb, struct gpio_pcal9539a_drv_data, gpio_callback);

	ARG_UNUSED(pins);

	/* Cannot read PCAL9539A registers from ISR context, queue worker */
	k_work_submit(&drv_data->interrupt_worker);
}
#endif /* CONFIG_GPIO_PCAL9539A_INTERRUPT */

static int gpio_pcal9539a_pin_interrupt_configure(const struct device *dev,
						  gpio_pin_t pin,
						  ...
						  enum gpio_int_trig trig)
{
	...
	if (!IS_ENABLED(CONFIG_GPIO_PCAL9539A_INTERRUPT)
	...
	if ((mode != GPIO_INT_MODE_DISABLED) &&
		...
		LOG_ERR("PCAL9539A[0x%X]: output pin cannot trigger interrupt",
...
/*
		...
		ret = gpio_pin_interrupt_configure(int_gpio_dev, config->int_gpio_pin, (active ? GPIO_INT_EDGE_TO_ACTIVE : GPIO_INT_MODE_DISABLED));
		if (ret != 0) {
			LOG_ERR("PCAL9539A[0x%X]: failed to configure interrupt on pin %d (%d)", config->i2c_slave_addr, config->int_gpio_pin, ret);
			goto err;
		}
*/
		if ((mode & GPIO_INT_ENABLE) == GPIO_INT_ENABLE) {
			drv_data->reg_cache.input_latch |= BIT(pin);
			drv_data->reg_cache.int_mask &= ~BIT(pin);

			ret |= write_port_regs(dev, REG_INPUT_LATCH_PORT0, &drv_data->reg_cache.input_latch,
					drv_data->reg_cache.input_latch);
			ret |= write_port_regs(dev, REG_INT_MASK_PORT0, &drv_data->reg_cache.int_mask, drv_data->reg_cache.int_mask);
			if (ret != 0) {
				LOG_ERR("PCAL9539A[0x%X]: failed to configure interrupt on pin %d (%d)", config->i2c_slave_addr,
...
//		drv_data->interrupt_active = active;
		...
		if (active) {
			/* Read current status to reset any active signal on INT line */
...
#ifdef CONFIG_GPIO_PCAL9539A_INTERRUPT
static int gpio_pcal9539a_manage_callback(const struct device *dev,
...
static const struct gpio_driver_api gpio_pcal9539a_drv_api_funcs = {
	.pin_configure = gpio_pcal9539a_pin_configure,
	.port_get_raw = gpio_pcal9539a_port_get_raw,
	.port_set_masked_raw = gpio_pcal9539a_port_set_masked_raw,
	.port_set_bits_raw = gpio_pcal9539a_port_set_bits_raw,
	.port_clear_bits_raw = gpio_pcal9539a_port_clear_bits_raw,
	.port_toggle_bits = gpio_pcal9539a_port_toggle_bits,
	.pin_interrupt_configure = gpio_pcal9539a_pin_interrupt_configure,
#ifdef CONFIG_GPIO_PCAL9539A_INTERRUPT
	.manage_callback = gpio_pcal9539a_manage_callback,
#endif
};

/**
 * @brief Initialization function of PCAL9539A
 *
 * @param dev Device struct
 * @return 0 if successful, failed otherwise.
 */
static int gpio_pcal9539a_init(const struct device *dev)
{
	const struct gpio_pcal9539a_config * const config = dev->config;
	struct gpio_pcal9539a_drv_data * const drv_data =
		(struct gpio_pcal9539a_drv_data * const)dev->data;
	...
	drv_data->reg_cache.input_latch = 0x0000;
	drv_data->reg_cache.int_mask = 0xFFFF;
	drv_data->reg_cache.int_status = 0x0000;
	...
	if ((config->capabilities & PCA_HAS_INTERRUPT) != 0) {
		...
		// 	LOG_ERR("PCAL9539A[0x%X]: error getting interrupt GPIO"
		...
		// ret |= gpio_pin_interrupt_configure(int_gpio_dev, config->int_gpio_pin,
		// 		(config->int_gpio_flags | GPIO_INPUT));
		ret = gpio_pin_configure(config->int_gpio.port, 
								 config->int_gpio.pin, 
		...
		if (ret != 0) {
			LOG_ERR("PCAL9539A[0x%X]: failed to configure interrupt"
...
static const struct gpio_pcal9539a_config gpio_pcal9539a_##inst##_cfg = {	\
	...
	.capabilities =	(DT_INST_PROP(inst, has_pud) ? PCA_HAS_PUD : 0) |				\
	 	 	IF_ENABLED(CONFIG_GPIO_PCAL9539A_INTERRUPT, ((DT_INST_NODE_HAS_PROP(inst, interrupt_gpios) ?		\
			PCA_HAS_INTERRUPT : 0) | ))	0,							\
