...
/**
 * thermistor_lookup_comp() - Finds indicies where ohm falls between
 *
 * @ohm: key value search is looking for
 * @i_low: return Lower interval index value
 * @i_high: return Higher interval index value
 */
static void thermistor_lookup_comp(const struct ntc_type *type, unsigned int ohm, int *i_low, int *i_high)
{
	int low = 0;
...
/**
 * thermistor_get_ohm() - Calculate the resistance read from NTC Thermistor
 *
...
int32_t thermistor_get_temp_mc(const struct ntc_type *type, unsigned int ohm)
{
	...
	thermistor_lookup_comp(type, ohm, &low, &high);
	/*
	 ...
	 */
	temp = fixp_linear_interpolate(type->comp[low].ohm, type->comp[low].temp_c * 1000,
