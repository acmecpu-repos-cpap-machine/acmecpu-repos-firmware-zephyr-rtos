...
/* checksum verification consumes memory, so we are keepig it disabled for the STSPIN32F0A driver chip
 * else the FLASH memory is overflowing */
 ...
 */
int lib_m2m_frame_decode(uint8_t *sbuf, uint32_t len, struct m2m_frame_t *frame);
