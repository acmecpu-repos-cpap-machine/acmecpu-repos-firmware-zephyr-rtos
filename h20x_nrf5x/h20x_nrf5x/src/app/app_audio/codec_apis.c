...
int CMX655SetLevel(uint8_t iLeft, uint8_t iRight)
{
    int     iStatus;
    int     iLevel;
...
int codec_enable_streams()
{
    ...
    #if 0
	// set format
	uint32_t fmt = (SND_SOC_DAIFMT_CBS_CFS | SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_NB_NF);
	ret = api->fmt_set(codec_dev, fmt);
    if (ret < 0) {
        LOG_ERR("fmt_set failed, fmt = %d", fmt);
        return ret;
    }

        // set record and playback params
    CMX655SetLevel(12,12);        /* 3dB gain */
    CMX655SetRvf(CMX655_VF_DCBLOCK /*| CMX655_VF_LPFEN_SHIFT*/);
    // CMX655SetNoiseGateAtten(CMX655_NGCTRL_EN);
    CMX655SetPvf(CMX655_VF_DCBLOCK);
    CMX655SetVolume(92);
...
int codec_init(const struct device *dev, int n_chan, int s_rate)
{
	...
	// set hw_params
	struct cmx655d_hw_params hw_params;
	hw_params.channels = n_chan;
	hw_params.sRate = s_rate;
	ret = api->hw_params_set(codec_dev, &hw_params);
    if (ret < 0) {
        LOG_ERR("hw_params_set failed, channels = %d, sRate = %d", hw_params.channels, hw_params.sRate);
        return ret;
    }
...
int codec_clock_setup(const struct device *dev, int n_chan, int s_rate)
{
    ...
    api = codec_dev->api;
   	...
   	// set system clock
	ret = api->sysclk_set(codec_dev, CMX655_SYSCLK_AUTO);
    if (ret < 0) {
        LOG_ERR("sysclk_set failed, clkid = %d", CMX655_SYSCLK_AUTO);
        return ret;
    }
	...
	hw_params.channels = n_chan;
	hw_params.sRate = s_rate;
    hw_params.clock_mode = 0;
    ret = api->main_clock_config(codec_dev, &hw_params);
    if (ret < 0) {
        LOG_ERR("main_clock_config failed");
        return ret;
    }
